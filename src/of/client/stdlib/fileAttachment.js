...
export function FileAttachment(name, base = location) {
  ...
  let info;
  if (typeof name === "object" && name && "name" in name) {
    info = name;
    name = name.name;
  }
  ...
  if (info) {
    return registerFile(name, info, base);
  }
...
export class AbstractFile {
  ...
  async parquet() {
    ...
    return Arrow.tableFromIPC(
      Parquet.readParquet(new Uint8Array(buffer)).intoIPCStream(),
    );
  }
  async sqlite() {
  ...
  async zip() {
    const [{ ZipArchive }, buffer] = await Promise.all([
      import("observablehq:stdlib/zip"),
      this.arrayBuffer(),
    ]);
  ...
  async xlsx() {
    const [{ Workbook }, buffer] = await Promise.all([
      import("observablehq:stdlib/xlsx"),
      this.arrayBuffer(),
    ]);
