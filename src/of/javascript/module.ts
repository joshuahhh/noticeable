...
function getModuleHashInternal(
  root: string,
  path: string,
  getHash = (p: string) => getFileHash(root, p),
): Hash {
...
export async function getLocalModuleHash(
  ...
  if (info) {
    ...
    for (const i of [
      ...
      ...info.globalDynamicImports,
    ]) {
      if (builtins.has(i) || i.startsWith("observablehq:")) {
        hash.update(
          `${resolveBuiltin(i)}?version=${process.env.npm_package_version}`,
        ); // change hash when Framework changes
    ...
    for (const p of globalPaths) {
      hash.update(p);
      for (const i of await parseImports(
        join(root, ".observablehq", "cache"),
        p,
      )) {
        if (i.type === "local") {
...
export async function transformJavaScript(
  ...
  return (await transform(source, getTransformOptions(loader, sourcePath)))
    .code;
}

export function transformJavaScriptSync(
  source: string,
  loader: "ts" | "jsx" | "tsx",
  sourcePath?: string,
): string {
